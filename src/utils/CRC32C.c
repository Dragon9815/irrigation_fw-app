...
//
// An implementation using a table with 256 elements
// Approx Code size = 1.1 kByte
//---------------------------------------------------------------------------------------
...
const uint32_t CRC32C_Table[ 256 ] = { 0x00000000, 0xF26B8303, 0xE13B70F7,
...
{
    return CRC32C_StartComputeBlock( aAddress, aLength ) ^
        0xFFFFFFFF; // Final XOR, CRC-32C Specification
}
...
uint32_t CRC32C_StartComputeBlock( const uint8_t * aAddress, uint32_t aLength )
{
    ...
    aCRC = 0xFFFFFFFF; // Initial Value, CRC-32C Specification

    while( aLength != 0 )
    {
        // aCRC = CRC32C_Table[((uint8_t) aCRC ^ *(aAddress))] ^ (aCRC >> 8);
        aCRC = CRC32C_Table[ ( aCRC ^ ( *aAddress ) ) & 0xFF ] ^ ( aCRC >> 8 );
        aAddress++;
        aLength--;
    }
...
{
    return CRC32C_AddByte( 0xFFFFFFFF, aByte );
}


uint32_t CRC32C_AddByte( uint32_t aCRC, uint8_t aByte )
{
    return CRC32C_Table[ ( aCRC ^ aByte ) & 0xFF ] ^ ( aCRC >> 8 );
}


uint32_t CRC32C_EndComputeBlock( uint32_t aCRC )
{
    return aCRC ^ 0xFFFFFFFF; // Final XOR, CRC-32C Specification
}
